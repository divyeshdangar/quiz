    ...
    <div class="card bg-white border-0 rounded-10 mb-4">
        <div class="card-body p-4">
            <div class="d-sm-flex text-center justify-content-between align-items-center border-bottom pb-20 mb-20">
                <h4 class="fw-semibold fs-18 mb-sm-0">{{ __('dashboard.task') }}</h4>
                <a href="{{ route('dashboard.task.create') }}" class="border-0 btn btn-primary py-2 px-4 text-white fs-14 fw-semibold rounded-3">
                    <span class="py-sm-1 d-block">
                        <i class="ri-add-line text-white"></i>
                        <span>{{ __('dashboard.add') }} {{ __('dashboard.task') }}</span>
                    </span>
                </a>
            </div>
                        ...
                        <tbody>
                            @foreach ($dataList as $data)
                                <tr>
                                    ...
                                    <td>
                                        <a target="_blank" href="{{ URL::asset('/images/task/'.$data->image) }}">
                                            <img src="{{ URL::asset('/images/task/'.$data->image) }}" width="250px" class="img-thumbnail">
                                        </a>
                                    </td>
                                    <td class="ps-0">
                                    ...
                                    <td>
                                        <div class="dropdown action-opt">
                                            <a class="btn bg p-1" href="{{ route('dashboard.task.view', ['id' => encrypt($data->id)]) }}">
                                                <i data-feather="eye"></i>
                                            </a>
                                            <a class="btn bg p-1" href="{{ route('dashboard.task.edit', ['id' => encrypt($data->id)]) }}">
                                                <i data-feather="edit-3"></i>
                                            </a>
                                            <a class="btn bg p-1" href="{{ route('dashboard.task.list', ['id' => encrypt($data->id)]) }}">
                                                <i data-feather="list"></i>
                                            </a>
                                            <a class="btn bg p-1" onclick="confirmAndDelete('{{ route('dashboard.task.delete', ['id' => encrypt($data->id)]) }}')">
